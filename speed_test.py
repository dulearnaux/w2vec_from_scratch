...
if __name__ == '__main__':
    ...
    # Sgram_NEG tests
    print('\nSgram Neg - Testing forward_neg, backward, loss and opt speed:')
    sgram_ns = w2v_numpy.SgramNS(vocab, VECTOR_DIM, window, batch, seed=1)
    data_ns = w2v_numpy.Dataloader(train_data, cbow_ns.window, negative_samples=5)
    neg_words = data_ns.neg_samples(context, target)
    tmp = timeit.Timer('sgram_ns.forward_neg(target, context, neg_words)', globals=globals()).repeat(1, 10)
    print(f'forward_neg:{tmp[0] / 10:.3}s per run')
    tmp = timeit.Timer('sgram_ns.loss_fn_neg()', globals=globals()).repeat(1, 10)
    print(f'loss_fn_neg:{tmp[0] / 10:.3}s per run')
    tmp = timeit.Timer('sgram_ns.backward()', globals=globals()).repeat(1, 10)
    print(f'backward:{tmp[0] / 10:.3}s per run')
    tmp = timeit.Timer('sgram_ns.forward_neg_quick(target, context, neg_words)', globals=globals()).repeat(1, 10)
    print(f'forward_neg_quick:{tmp[0] / 10:.3}s per run')
    cbow_ns.loss_fn_neg()
    tmp = timeit.Timer('sgram_ns.backward_neg_quick()', globals=globals()).repeat(1, 10)
    print(f'backward_neg_quick:{tmp[0] / 10:.3}s per run')
    tmp = timeit.Timer('sgram_ns.backward_neg_quickest()', globals=globals()).repeat(1, 10)
    print(f'backward_neg_quickest:{tmp[0] / 10:.3}s per run')
    tmp = timeit.Timer('sgram_ns.optim_sgd(0.00001)', globals=globals()).repeat(1, 10)
    print(f'optim_sgd:{tmp[0] / 10:.3}s per run')
    print('\n')
